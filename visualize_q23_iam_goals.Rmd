...
In this R notebook, we analyse survey data related to goals set for Identity and Access Management (IAM) by organizations. Several visualization methods are used including: likert chart, pie chart and multi-histograms.  

## Introduction

As part of the *IAM Performance Measurement 2020 Survey*, I surveyed IAM professionals about the goals and priorities set for IAM by organizations.

The original survey question #23 was:

> What are the key strategic goals set for IAM by top management in your organization?

The notebook focuses on data analysis. The business implications will be discussed in a distinct article.
...
q23_goal_options_url = "https://raw.githubusercontent.com/Open-Measure/Open-Data/master/IAMPerf2020-Dataset/IAMPerf2020Q23GoalOptions.csv";
q23_priority_options_url = "https://raw.githubusercontent.com/Open-Measure/Open-Data/master/IAMPerf2020-Dataset/IAMPerf2020Q23PriorityOptions.csv";
...
q23_goal_options <- read.csv (text = RCurl::getURL(q23_goal_options_url));
q23_priority_options <- read.csv (text = RCurl::getURL(q23_priority_options_url));
...
goals_count = nrow(q23_goal_options); 

# Have a quick loot at the goal categories
knitr::kable(q23_goal_options);
...
q23_priority_options = q23_priority_options[q23_priority_options$X != priority_na_value, ];

# And let's have a quick look at the data
knitr::kable(q23_priority_options);
...
# Apply nicely labeled and properly ordered factors.
  question_data$Q23R1 = factor(question_data$Q23R1, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R2 = factor(question_data$Q23R2, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R3 = factor(question_data$Q23R3, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R4 = factor(question_data$Q23R4, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R5 = factor(question_data$Q23R5, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R6 = factor(question_data$Q23R6, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R7 = factor(question_data$Q23R7, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R8 = factor(question_data$Q23R8, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R9 = factor(question_data$Q23R9, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R10 = factor(question_data$Q23R10, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
  question_data$Q23R11 = factor(question_data$Q23R11, levels = q23_priority_options$X, labels = q23_priority_options$Title, ordered = TRUE, exclude = NA);
...
colnames(question_data) = q23_goal_options$Title;
...
knitr::kable(head(priorities_table));
...
I consider that organizations with more than 3 IAM primary goals have an improperly defined or undecisive strategy. Let's call this category "**Scattered**".
...
scattered_strategy_threshold = 4;
...
priorities_table$Strategy = ifelse(
  priorities_table$PrimaryGoal >= focused_strategy_threshold & 
  priorities_table$PrimaryGoal < scattered_strategy_threshold,
  "Focused", 
...
We are now ready to visualize the data. To start with, let's use an old-style pie chart to get a feeling of the ratio of organization having a **scattered**, **unknown** or **focused** IAM strategy.
...
factor(
  x = pie_data$Strategy, 
  levels = c("Scattered", "Unknown", "Focused"),
  exclude = NA,
  ordered = TRUE);
...
strategy_colors = c("Scattered" = "#ff6600", "Unknown" = "#dddddd", "Focused" = "#0066ff");
...
ggplot2::ggplot(
 ...
 ggplot2::geom_text(
   ggplot2::aes(
     label = paste(round(Frequency / sum(Frequency) * 100, 1), "%")),
...
knitr::kable(head(all_priorities));
...
As you can see, the **scattered**, **unknown** and **focused** strategies are displayed using color codes. This gives a great visual intuition of how these strategies compose the frequency distributions.
